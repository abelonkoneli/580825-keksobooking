...
var checkType = function (key) {
  for (var i = 0; i <= 7; i++) {
  if (key === 'Большая уютная квартира' || key === 'Маленькая неуютная квартира') {type = 'flat';}
  else if (key === 'Огромный прекрасный дворец' || key === 'Маленький ужасный дворец' || key === 'Красивый гостевой домик' || key === 'Некрасивый негостеприимный домик') {type = 'house';}
    else {type = 'bungalo';}
} return type;
};
...
var buttonElement = pointerElement.querySelector('.map__pin'); //здесь перестает работать remove('map--faded')
...
switch(proposals[i].offer.features[j]) {
  ...
  break;
  case 'dishwasher':
  var itemFeatures = featuresContainer.querySelector('feature--dishwasher').cloneNode(true),
  featuresContainer.appendChild(itemFeatures);
  break;
  case 'parking':
  var itemFeatures = featuresContainer.querySelector('feature--parking').cloneNode(true),
  featuresContainer.appendChild(itemFeatures);
  break;
  case 'washer':
  var itemFeatures = featuresContainer.querySelector('feature--washer').cloneNode(true),
  featuresContainer.appendChild(itemFeatures);
  break;
  case 'elevator':
  var itemFeatures = featuresContainer.querySelector('feature--elevator').cloneNode(true),
  featuresContainer.appendChild(itemFeatures);
  break;
  case 'conditioner':
  var itemFeatures = featuresContainer.querySelector('feature--conditioner').cloneNode(true),
  featuresContainer.appendChild(itemFeatures);
  break;
}
}
...
for (var j = 1; j <= proposals[0].offer.pictures.length; j++) {
